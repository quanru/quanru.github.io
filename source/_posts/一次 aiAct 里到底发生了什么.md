...
上一篇《[为什么 Midscene 的 UI Agent 非得看见屏幕？](/2026/05/26/为什么-UI-Agent-非得看见屏幕/)》讲了 Midscene 为什么把"看截图"放在 UI 操作的最前面。但讲完那个判断之后，我经常被同事问下一个问题：
...
Locate 任务负责把"登录按钮"这个描述变成精确像素坐标，Tap 任务拿到坐标后才真的点。这一步是 Midscene 工程化里最关键的拆分之一，但展开讲会喧宾夺主——下一篇《[Midscene 为什么要把 Locate 与 Action 拆成两步？](/2026/05/26/Locate-与-Action-为什么要拆成两步/)》会单独写。
...
下一步该深挖的是循环里最关键的那一站——"找元素"。AI 给的那个 `bbox` 为什么不够准？四级 Fallback 链是怎么从"免费估算"一路降本提精到"昂贵的精确定位"？这部分放在下一篇 [Midscene 为什么要把 Locate 与 Action 拆成两步？](/2026/05/26/Locate-与-Action-为什么要拆成两步/) 里讲。
