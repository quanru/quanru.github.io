...
（这里先澄清一个容易误会的点：bbox 并不是模型每一轮都会给。只有满足特定条件时，规划阶段的提示词才会**要求**模型把 bbox 一起返回；一旦用户表达"我要更准"，提示词就不再要这个字段。具体条件下一节第 1 级会展开。所以下文说的"bbox 不准"，针对的是"模型给了 bbox 的那些场景"。）
...
这样一来，**Action 任务自己永远不碰 AI 给的粗糙 bbox**——它拿到的总是 Locate 任务输出的坐标。

这里要特别说清一层，否则下一节会读着矛盾：Action 层只认 Locate 任务的输出，但这个坐标**怎么来的**——是直接采信 bbox、还是走 DOM 查询、历史复用、甚至重新发一次 AI 定位——全部封装在 Locate 任务内部（就是下一节的四级 Fallback）。换句话说，**bbox 到底用不用，是 Locate 这一层的事，Action 层无需关心**。所以"Action 不碰 bbox"和"系统在某些情况下确实会用 bbox"两句话同时成立，只是主语不同：前者说的是 Action 任务，后者说的是 Locate 任务的第 1 级。
...
视觉 Agent 真正难的不是"点击"，是"找到"。

### 反过来想：如果 Locate 和 Action 不拆成两步，会怎样？

如果不拆，会有四类问题，核心都源于"让规划模型顺手给的 bbox 直接驱动点击"：

1. **定位精度被规划任务拖累，且无法独立优化。** 不拆的话，定位坐标只能来自规划模型"一心四用"时顺手给的 bbox（它同时在理解意图、选动作类型、生成参数、估位置）。实测表现：大按钮（> 80px）够用，中等元素（40-80px）偏 20-50 像素，小按钮（< 40px）可能完全偏出、点到隔壁。拆开后，Locate 成为独立任务，可以单独上更准的策略（独立 AI locate、deepLocate 两段定位），而不必去砸规划模型那块"能力天花板"。

2. **错误会延迟暴露，循环空转。** 直接拿 bbox 去点，点错了程序也认为"成功"了——要等下一轮重新截图、AI 发现界面没按预期变化，才意识到出错。这种延迟感知让整个 plan-act-replan 循环跑很多无用功。

3. **失去四级 Fallback 这套成本分层。** 拆分是 Fallback 链的前提。不拆就只有"信任 bbox"一条路，没法做 Plan hit → XPath → Cache → AI locate 的"命中即停、成本从低到高"分层：简单页面省不掉 AI 调用，复杂页面也没有像素级 DOM / 缓存定位兜底。

4. **缓存与稳定回归失效。** XPath hit / Cache hit 这两级零 AI 调用的像素级定位，依赖把定位独立出来才能写入和复用。不拆就拿不到稳定的历史定位特征，CI 里的 read-only 缓存（保证跑"上次审过的路径"）也就无从谈起，定位结果会随 AI 每次心血来潮而漂移。

一句话：拆分把"找到"从"执行"里解耦，让定位可以独立加策略、可缓存、可分层兜底，并且让 Action 任务永远只拿 Locate 输出的坐标，而不是自己去消费粗糙的 bbox 估算。

Midscene 在这一点上的工程取舍可以总结成三句话：

1. **拆开**——Locate 和 Action 拆成两步，Action 任务自己永远不碰 AI 的 bbox，只消费 Locate 任务输出的坐标；bbox 用不用、怎么用，全封装在 Locate 内部那一级里
